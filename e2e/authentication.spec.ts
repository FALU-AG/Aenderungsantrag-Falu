...
test("Anmeldung, Benutzerverwaltung, Zugriff und AVOR-Einkauf",async({page})=>{
  await page.goto("./"); await expect(page).toHaveURL(/\/aenderungsantrag\/login$/);
  ...
  await page.goto("admin/users"); await expect(page).toHaveURL(/\/aenderungsantrag\/?$/); await logout(page);
