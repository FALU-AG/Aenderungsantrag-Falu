...
const mocks=vi.hoisted(()=>({ensureLocalUser:vi.fn(),consume:vi.fn()}));
vi.mock("@/server/db/client",()=>({db:{$transaction:async(f: (tx: unknown)=>unknown)=>f({appAssertionUse:{deleteMany:vi.fn(),create:mocks.consume}})}}));
vi.mock("@/modules/auth/provisioning",()=>({ensureLocalUser:mocks.ensureLocalUser}));
...
beforeEach(()=>{process.env.FALU_APP_SIGNING_PUBLIC_KEY=keys.publicKey;mocks.ensureLocalUser.mockReset().mockResolvedValue({id:"local-a"});mocks.consume.mockReset().mockResolvedValue({});});
describe("cryptographic origin protection",()=>{
 ...
 // Provisioning only trusts the signed directory, so an identity it does not list gets no row.
 it("refuses a verified identity the portal directory does not list",async()=>{mocks.ensureLocalUser.mockResolvedValue(null);const r=await proxy(new NextRequest("https://railway.invalid/aenderungsantrag",{headers:{"x-falu-assertion":assertion()}}));expect(r.status).toBe(403)});
