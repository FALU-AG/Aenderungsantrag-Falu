...
export function LoginForm() {
  ...
  return <form action={action} className="mt-8 space-y-5">
    <div><label htmlFor="email" className="block text-sm font-medium text-slate-700">E-Mail</label><input id="email" name="email" type="email" autoComplete="username" autoFocus className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 focus:border-[#175f91] focus:outline-none focus:ring-2 focus:ring-[#175f91]/25" />{state.errors?.email && <p className="mt-1 text-sm text-red-700">{state.errors.email}</p>}</div>
    <PasswordInput id="password" label="Passwort" name="password" autoComplete="current-password" error={state.errors?.password} />
    <div className="text-right"><Link href="/forgot-password" className="text-sm font-medium text-[#175f91] hover:underline focus:outline-none focus:ring-2 focus:ring-[#175f91]">Passwort vergessen?</Link></div>
