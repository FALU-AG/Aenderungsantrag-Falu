...
describe("responsive AppShell",()=>{
  ...
  it("zeigt Stellvertretung nur Benutzern mit eigener delegierbarer Freigaberolle",async()=>{const employee=await AppShell({user:{id:"u1",name:"Erika",email:"e@falu.test",roles:["EMPLOYEE"]},children:null});const employeeView=render(employee);expect(screen.queryByRole("link",{name:"Stellvertretung"})).not.toBeInTheDocument();employeeView.unmount();const avor=await AppShell({user:{id:"u2",name:"Anna",email:"a@falu.test",roles:["AVOR"]},children:null});render(avor);expect(screen.getAllByRole("link",{name:"Stellvertretung"}).length).toBeGreaterThan(0)});
});
