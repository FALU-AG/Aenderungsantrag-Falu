...
function InboxRow({item}:{item:InboxItem}){return <Link href={item.href} className="grid gap-3 px-4 py-4 transition-colors hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[#175f91] sm:px-5 lg:grid-cols-[100px_150px_minmax(240px,1fr)_120px_170px_110px] lg:items-center lg:gap-4"><div className="flex items-center justify-between gap-2 lg:block"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{item.typeLabel}</span><span className={`rounded-full px-2.5 py-1 text-xs font-semibold lg:hidden ${item.overdue?"bg-red-100 text-red-700":"bg-blue-50 text-[#175f91]"}`}>{item.overdue?"Überfällig":item.statusLabel}</span></div><div><p className="font-mono text-sm font-semibold text-[#175f91]">{item.requestNumber}</p><p className="mt-0.5 text-sm text-slate-600 lg:truncate lg:text-xs lg:text-slate-500">{item.requestTitle}</p></div><div><p className="text-sm font-semibold text-slate-900">{item.action}</p>{item.delegatedFor&&<p className="mt-1 text-xs font-medium text-amber-700">Stellvertretung für {item.delegatedFor}</p>}</div><p className="text-sm text-slate-600"><span className="text-xs text-slate-500 lg:hidden">Maschinentypen: </span>{item.machineTypes.join(", ")||"–"}</p><div className="text-sm text-slate-600">{item.dueDate?<p className={item.overdue?"font-semibold text-red-700":""}><span className="text-xs font-normal text-slate-500 lg:hidden">Fällig / Priorität: </span>{date(item.dueDate)}{item.priority?` / ${PRIORITY_LABELS[item.priority]}`:""}</p>:<span className="hidden lg:inline">–</span>}</div><div className="hidden lg:block"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.overdue?"bg-red-100 text-red-700":"bg-blue-50 text-[#175f91]"}`}>{item.overdue?"Überfällig":item.statusLabel}</span></div></Link>}
