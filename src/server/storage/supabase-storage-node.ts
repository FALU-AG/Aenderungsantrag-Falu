...
export type SupabaseStorageEnvironment = {
  SUPABASE_URL?: string;
...
export function createSupabaseStorageClient(environment?: SupabaseStorageEnvironment) {
  ...
  if (!url || !key) {
    throw new Error("Supabase Storage ist nicht konfiguriert. SUPABASE_URL und SUPABASE_SERVICE_ROLE_KEY werden benötigt.");
  }
...
export async function uploadSupabaseObject(client: StorageClient, key: string, data: Buffer, mimeType: string) {
  const { error } = await client.storage.from(ATTACHMENT_BUCKET).upload(key, data, {
