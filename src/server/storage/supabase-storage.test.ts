...
describe("Supabase Storage adapter",()=>{
  ...
  it("meldet Storage-Uploadfehler sicher",async()=>{await expect(uploadSupabaseObject(client({error:{message:"secret detail"}}).client,"key",Buffer.from("x"),"image/png")).rejects.toThrow("Der Anhang konnte nicht hochgeladen werden");});
  ...
  it("meldet fehlende Produktionskonfiguration ohne Secrets",()=>expect(()=>createSupabaseStorageClient({SUPABASE_URL:"",SUPABASE_SERVICE_ROLE_KEY:""})).toThrow("SUPABASE_URL und SUPABASE_SERVICE_ROLE_KEY"));
});
