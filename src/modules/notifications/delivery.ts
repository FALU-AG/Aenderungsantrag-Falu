...
import { createSlackProvider, type SlackProvider } from "./slack-provider";
...
export type NotificationDeliveryProvider = {
  send(payload: DeliveryPayload): Promise<{ id: string }>;
};
  ...
  return {
    async send(payload) {
      if (notificationChannel(payload.type) === "email") {
        ...
        return (options.email ?? createEmailProvider()).send({
