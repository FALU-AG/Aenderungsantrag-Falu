...
const row = { id: "n1", type: "TASK_ASSIGNED", status: "PENDING", attemptCount: 0, recipientEmail: "user@falu.ch", recipientName: "Erika Beispiel", subject: "Neue Aufgabe", idempotencyKey: "task:1", templateData: { title: "Zeichnung" } };
describe("notification delivery", () => {
  ...
  it("passes the stored original recipient identity through the shared delivery boundary", async () => { await sendNotification("n1"); expect(providerSend).toHaveBeenCalledWith(expect.objectContaining({recipientEmail:"user@falu.ch",recipientName:"Erika Beispiel"})); });
