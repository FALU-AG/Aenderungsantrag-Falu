...
export async function runWeeklyDigest(options: { now?: Date; ignoreSchedule?: boolean } = {}) {
  ...
  const [users, pendingApprovals] = await Promise.all([
    db.user.findMany({ where: { id: { in: directory.map((u)=>u.id) } }, select: {
      id: true, email: true, name: true,
      delegationsReceived: { where: { enabled: true, startsAt: { lte: now }, endsAt: { gte: now } }, orderBy: [{ startsAt: "desc" }, { createdAt: "desc" }, { id: "asc" }], select: { scope: true, delegatingUser: { select: { id: true, name: true } } } },
  ...
  for (const user of users) {
    ...
    const explicitRoles = new Set(directory.find((u)=>u.id===user.id)!.roles.map(({ role }) => role.key));
    const approvals = new Map<string, { number: string; title: string; types: string[]; url: string }>();
    for (const approval of pendingApprovals) {
      const delegatedFor = user.delegationsReceived?.find((delegation) => delegation.scope === `${approval.type}_APPROVAL` && directory.find((u)=>u.id===delegation.delegatingUser.id)?.roles.some(({ role }) => role.key === approval.type))?.delegatingUser.name;
