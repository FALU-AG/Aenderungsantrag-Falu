...
async function requestSummary(tx: Prisma.TransactionClient, id: string) {
  return tx.changeRequest.findUniqueOrThrow({ where: { id }, select: { id: true, number: true, title: true, applicantName: true, approvalCycle: true, finalReviewCycle: true } });
}
