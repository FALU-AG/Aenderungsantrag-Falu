...
import { DELEGATION_SCOPE_LABELS, delegationInputSchema, parseZurichDateTimeLocal, roleForScope } from "./domain";
...
export async function updateDelegation(id: string, formData: FormData) {
  const actor = await getCurrentUser();
  const existing = await db.approvalDelegation.findUniqueOrThrow({ where: { id } });
