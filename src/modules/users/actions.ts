...
import { hashPassword, validateNewPassword } from "@/modules/auth/password";
...
export async function createUser(form: FormData) {
  const actor = await requireRole("ADMINISTRATOR"); const data = values(form); const password = String(form.get("password") ?? ""); const confirmation = String(form.get("passwordConfirmation") ?? "");
  if (!data.firstName || !data.lastName || !data.email || !data.roles.length) throw new Error("Bitte füllen Sie alle Pflichtfelder aus.");
  const passwordErrors = validateNewPassword(password, confirmation); if (passwordErrors.password || passwordErrors.passwordConfirmation) throw new Error(passwordErrors.password ?? passwordErrors.passwordConfirmation);
...
export async function resetPassword(userId: string, form: FormData) {
  const actor = await requireRole("ADMINISTRATOR"); const password = String(form.get("password") ?? ""); const confirmation = String(form.get("passwordConfirmation") ?? ""); const passwordErrors = validateNewPassword(password, confirmation); if (passwordErrors.password || passwordErrors.passwordConfirmation) throw new Error(passwordErrors.password ?? passwordErrors.passwordConfirmation);
