...
async function main() {
  ...
  const samples = [
    ...
    { number: "CR-2026-007", title: "Optimierung Einlaufblech CB1", applicantId: "sample-max-muster", machineTypeId: machineId("CB1"), description: "Technische Ausarbeitung des angepassten Einlaufblechs.", status: "AVOR_PRODUCTION_PREPARATION" as const, reasonIndexes: [1] },
    { number: "CR-2026-008", title: "Neue Lagerung Förderwelle CT", applicantId: "sample-anna-avor", machineTypeId: machineId("CT"), description: "Die Lagerung der Förderwelle wird technisch überarbeitet.", status: "PURCHASING_PROCUREMENT" as const, reasonIndexes: [1, 7] },
    { number: "CR-2026-009", title: "Anpassung Kupplungsaufnahme SV-2X", applicantId: "sample-max-muster", machineTypeId: machineId("SV-2X"), description: "Die neue Kupplungsaufnahme ist nicht rückwärts austauschbar.", status: "PURCHASING_PROCUREMENT" as const, reasonIndexes: [0] },
    { number: "CR-2026-010", title: "Sicherheitsbügel Baugruppe BL-16", applicantId: "sample-thomas-technik", machineTypeId: machineId("BL-16"), description: "Der Sicherheitsbügel beeinflusst weitere Baugruppen.", status: "PURCHASING_PROCUREMENT" as const, reasonIndexes: [8] },
    { number: "CR-2026-011", title: "Nachrüstsatz Schutzsensor ABS", applicantId: "sample-max-muster", machineTypeId: machineId("ABS"), description: "Für ausgelieferte Anlagen wird ein Nachrüstsatz vorgesehen.", status: "PURCHASING_PROCUREMENT" as const, reasonIndexes: [2, 8] },
  ];
  ...
  for (const number of ["CR-2026-007", "CR-2026-008", "CR-2026-009", "CR-2026-010", "CR-2026-011"]) { await decision(number, "AVOR", "APPROVED", "sample-anna-avor"); await decision(number, "TECHNICAL", "APPROVED", "sample-thomas-technik"); }
  ...
  await prisma.technicalReview.upsert({ where: { changeRequestId: seeded.get("CR-2026-011")!.id }, update: {}, create: { changeRequestId: seeded.get("CR-2026-011")!.id, ...reviewBase, completed: true, completedById: "sample-thomas-technik", completedAt: new Date() } });
  const avorBase = { stockNeedsAction: "NO" as const, purchaseOrdersNeedUpdate: "NO" as const, productionOrdersNeedUpdate: "NO" as const, deliveredMachinesNeedParts: "NO" as const, currency: "CHF", remarks: "AVOR-Auswirkungen geprüft." };
  await prisma.avorImpactReview.upsert({ where: { changeRequestId: seeded.get("CR-2026-007")!.id }, update: {}, create: { changeRequestId: seeded.get("CR-2026-007")!.id, stockNeedsAction: "NO", remarks: "Lagerbestand wird noch geprüft." } });
  await prisma.avorImpactReview.upsert({ where: { changeRequestId: seeded.get("CR-2026-008")!.id }, update: {}, create: { changeRequestId: seeded.get("CR-2026-008")!.id, ...avorBase, completed: true, completedById: "sample-anna-avor", completedAt: new Date() } });
  await prisma.avorImpactReview.upsert({ where: { changeRequestId: seeded.get("CR-2026-009")!.id }, update: {}, create: { changeRequestId: seeded.get("CR-2026-009")!.id, ...avorBase, stockNeedsAction: "YES", stockActionExplanation: "Vorhandene Kupplungsaufnahmen nacharbeiten und Restbestand umbuchen.", productionOrdersNeedUpdate: "YES", productionOrderExplanation: "Zwei gerüstete Aufträge auf den neuen Zeichnungsstand umstellen.", completed: true, completedById: "sample-anna-avor", completedAt: new Date() } });
  await prisma.avorImpactReview.upsert({ where: { changeRequestId: seeded.get("CR-2026-010")!.id }, update: {}, create: { changeRequestId: seeded.get("CR-2026-010")!.id, ...avorBase, purchaseOrdersNeedUpdate: "YES", purchaseOrderExplanation: "Offene Bestellung der Haltewinkel auf neue Revision ändern.", completed: true, completedById: "sample-anna-avor", completedAt: new Date() } });
  await prisma.avorImpactReview.upsert({ where: { changeRequestId: seeded.get("CR-2026-011")!.id }, update: {}, create: { changeRequestId: seeded.get("CR-2026-011")!.id, ...avorBase, deliveredMachinesNeedParts: "YES", deliveredMachinesExplanation: "Nachrüstsätze für betroffene ausgelieferte Anlagen bereitstellen.", validFromMachineNumber: "11850", estimatedAdditionalCosts: 1250, completed: true, completedById: "sample-anna-avor", completedAt: new Date() } });
  await prisma.changeRequestCounter.upsert({ where: { year: 2026 }, update: { nextNumber: { set: 12 } }, create: { year: 2026, nextNumber: 12 } });
}
